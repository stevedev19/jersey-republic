// Enhanced Signup Page JavaScript - Modern Interactive Features
console.log("Enhanced Signup Page JavaScript initialized");
...
function validateSignForm() {
  ...
  if (isValid) {
    showNotification('Form validation successful! Creating account...', 'success');
...
function initImageUpload() {
  ...
  if (!fileInput || !fileName || !imagePreview) return;
  ...
  switch (fieldName) {
    ...
    case 'memberPhone':
      if (value && !isValidPhone(value)) {
        showFieldError(`.${field.className}`, 'Please enter a valid phone number');
      }
      break;
    case 'memberPassword':
    ...
    case 'confirmPassword':
      ...
      if (value && value !== password) {
        showFieldError(`.${field.className}`, 'Passwords do not match');
      }
      break;
  }
}
...
document.addEventListener('DOMContentLoaded', function() {
  console.log('Signup page DOM loaded, initializing features...');
  ...
  const formElements = document.querySelectorAll('.sign-up-frame, .form-section');
  ...
  const interactiveElements = document.querySelectorAll('.form-input, .file-input-label, .back-btn');
  ...
  console.log('All signup page features initialized successfully!');
});
...
// Enhanced image preview handler - Same as Products
function previewFileHandler(input, index) {
  ...
  if (file) {
    ...
    overlay.innerHTML = `<i class="fas fa-camera"></i><span>Click to upload</span>`;
